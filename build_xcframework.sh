...
#
# MACH_O_TYPE=mh_dylib overrides the Swift Package's default static-library
# product type so that xcodebuild produces a proper .framework bundle inside
# the archive's Products/Library/Frameworks/ directory.  Without this flag,
# xcodebuild only generates a plain .a static library (installed under a
# user-specific Objects path) and the -create-xcframework step that follows
# cannot locate a .framework to assemble the XCFramework from.
# ---------------------------------------------------------------------------
build_archive() {
    local destination="$1"
